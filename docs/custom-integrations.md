...
RSS Gallery 支持自定义集成功能，允许你将 RSS 条目快速发送到第三方服务，或通过 URL 跳转实现快捷操作。
...
在添加或编辑 Feed 时，可以选择该 Feed 启用哪些集成。只有被勾选的集成才会显示在该 Feed 条目的工具栏中。
...
每个 Feed 可以独立控制启用哪些集成：

1. 在「添加 Feed」或「编辑 Feed」对话框中
2. 找到「启用的集成」部分
3. 勾选需要启用的集成

未勾选的集成不会出现在该 Feed 条目的工具栏中。

## 执行历史

在集成面板的「执行历史」标签页中，可以查看：
...
### Q: 如何在多个 Feed 使用相同的集成配置？

集成配置是全局的，在创建后可以在任意 Feed 中启用。每个 Feed 只需选择要启用的集成即可。
